...
export default function Eventos() {
  ...
  const { KONG_URL, user, setEventEdit } = useContext(GlobalContext);
  ...
  async function getEvents() {
    if (!user) return; // Retorna cedo se o usuário não existir
    setisFetching(true);
    try {
      const response = await fetch(`${KONG_URL}/companys/events/1`, {
        method: 'GET',
        headers: {
      ...
      setisFetching(false); // Sempre desativa o loading, independentemente do resultado
    }
  }
